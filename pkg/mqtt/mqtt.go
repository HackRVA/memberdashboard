...
func NewMQTTServer(subscriberPrefix string, publisherPrefix string) *mqttServer {
	return &mqttServer{}
}
...
func (m *mqttServer) Subscribe(topic string, handler mqtt.MessageHandler) {
	...
	opts.SetClientID(m.SubscriberPrefix + randStringRunes(12))
...
func (m *mqttServer) Publish(topic string, payload interface{}) {
	conf, _ := config.Load()
	opts := mqtt.NewClientOptions().AddBroker(conf.MQTTBrokerAddress)
	opts.SetClientID(m.PublisherPrefix + randStringRunes(12))
