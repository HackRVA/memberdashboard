...
func TestGetCommunication_Success(t *testing.T) {
	...
	got, err := db.GetCommunication("welcome")
	if err != nil {
...
func TestGetCommunication_Error(t *testing.T) {
	...
	_, err := db.GetCommunication("missing")
	if err == nil {
...
func TestGetMostRecentCommunicationToMember_Success(t *testing.T) {
	...
	got, err := db.GetMostRecentCommunicationToMember("mem-1", 3)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !got.Equal(now) {
...
func TestLogCommunication_Success(t *testing.T) {
	...
	if err := db.LogCommunication(7, "mem-1"); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("unmet expectations: %v", err)
